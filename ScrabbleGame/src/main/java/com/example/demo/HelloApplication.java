...
public class HelloApplication extends Application {
    ...
    private Consumer<Pair<String, Integer>> connectCallback;

    private static int MAX_PLAYERS = 2; // Maximum number of players
...
//                    System.out.println("IP address: " + addr.getHostAddress()); // check mi ip wifi address
                    ...
                    // Start the server
                     Backend.MyServer server = new MyServer(8080, new MyServer.ClientHandler1());
                    server.start();
        ...
        askForIp(newGameGuestButton, pair -> {
            try {
                Client client = new Client(pair.getKey(), pair.getValue());
                client.start();
            } catch (IOException e) {
        ...
        HBox buttonBox = new HBox(newGameHostButton, newGameGuestButton);
        ...
        Image backgroundImage = new Image("file:/Users/oripaso/Desktop/ScrabbleBook/demo/src/main/java/com/example/demo/scrabblePhoto.jpg");
        ...
        Text creditsText = new Text("By Yonatan, Tal, Shay, Ori");
        creditsText.setFont(Font.font("Arial", FontWeight.BOLD, 21));
        creditsText.setStyle("-fx-fill: RED; -fx-font-weight: bold; -fx-alignment: center;");
    ...
    private void askForIp(Button ipButton, Consumer<Pair<String, Integer>> connectCallback) {
        ...
        ipButton.setOnAction(e -> {
            ...
            if (result.isPresent()) {
                ...
                connectCallback.accept(new Pair<>(ip, port));
            }
        });
    }
    ...
    private int getNumberOfPlayers() {
        // TODO: Implement the logic to determine the number of players currently connected
        // For testing purposes, return a random number of players
        return (int) (Math.random() * MAX_PLAYERS);
    }

    private void openScrabbleGame() {
        // Create an instance of the ScrabbleGame class and call its start() method
